...
@section('content')
    ...
    <body class="font-[Poppins]">
        ...
        <header class="flex flex-col items-center gap-[50px] mt-[70px]">
            <div id="Headline" class="max-w-[1130px] mx-auto flex flex-col gap-4 items-center">
                ...
                <div class="flex items-center justify-center gap-[70px]">
                    <a id="Author" href="a{{ route('front.author', $articleNews->author->slug) }}" class="w-fit h-fit">
        ...
        <section id="Article-container" class="max-w-[1130px] mx-auto flex gap-20 mt-[50px] font-[Lexend Deca]">
                    ...
                    <a href="{{ route('front.details', $item_news->slug) }}" class="card-from-author">
                        <div
                            ...
                            <div class="flex flex-col gap-[6px]">
                                ...
                                <p class="text-xs leading-[18px] text-[#A3A6AE]">{{ $item_news->created_at->format('M b, Y') }} • {{ $item_news->category->name }}</p>
                            </div>
                        </div>
                    </a>
                    @empty
                        <p> Belum ada artikel baru lainnya</p>
                    @endforelse
                </div>
                <div class="ads flex flex-col gap-3 w-full">
                            ...
                            <div
                                ...
                                <div class="flex flex-col gap-[6px]">
                                    <h3 class="text-lg leading-[27px] font-bold">
                                        {{ substr($article->name, 0, 70) }}{{ strlen($article->name) > 50 ? '...' : '' }}</h3>
...
@endpush
@push('after-scripts')
    <script src="jsht/two-lines-text.js"></script>
@endpush
